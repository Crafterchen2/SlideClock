...
function updateTrackInfo(trackInfo) {
  if (trackInfo) {
    document.querySelector(".thumbnail").src =
      trackInfo.thumbnail ?? noTrackJpg;
    document.querySelector(".title").textContent =
      trackInfo.title ?? "Kein Titel";
    document.querySelector(".desc").textContent =
      trackInfo.artist ?? "Kein Künstler";
  } else {
...
function changeOrbColor(hexColorP, hexColorS, hexColorT) {
  ...
  let orb1rgb =
    hexColorP != null ? hexToRgb(hexColorP) : { r: 124, g: 58, b: 237 };
  let orb2rgb =
    hexColorS != null ? hexToRgb(hexColorS) : { r: 37, g: 99, b: 235 };
  let orb3rgb =
    hexColorT != null ? hexToRgb(hexColorT) : { r: 6, g: 182, b: 212 };
  ...
  setOrbColor(orb1, 1, orb1rgb);
  setOrbColor(orb2, 2, orb2rgb);
  setOrbColor(orb3, 3, orb3rgb);
  setOrbColor(orb4, 4, orb4rgb);
}

function setOrbColor(orb, index, rgb) {
...
function wallpaperMediaThumbnailListener(event) {
  if (mediaPlaying) {
    ...
    changePlayerBackgroundColor(
      event.primaryColor ?? "#000000",
      event.secondaryColor ?? "#000000",
    );
    if (settings.player.affectOrbs)
      changeOrbColor(
        event.primaryColor,
...
try {
  ...
  console.log(
    "sliderClock.js -> This doesn't seem to be the Wallpaper Engine.",
  );
}
